...
library(yaml)
#library(restorepoint)
setwd("D:/Studium/Bachelor/Bachelor-Arbeit/ProblemSet")
ps.name = "RTutorMediaPersuasion2"; sol.file = paste0(ps.name,"_sol.Rmd")
libs = c("foreign", "tidyr", "dplyr", "ggplot2", "lfe", "stargazer") # character vector of all packages you load in the problem set
#name.rmd.chunks(sol.file) # set auto chunk names in this file
create.ps(sol.file=sol.file, ps.name=ps.name, user.name=NULL,libs=libs, stop.when.finished=FALSE, var.txt.file = "variables.txt",
          extra.code.file = "functions2.R", addons = "quiz")
...
#< hint
  display("Just write: dat <- read.dta(\"NTV_Aggregate_Data.dta\")")
#>
```
...
#< quiz "Gorod"
question: What do you think does 'Gorod' mean?
sc:
  - transmitter
  - factory
  - city*
success: Correct! 'Gorod' is the russian word for 'city'.
failure: I'm sorry, your answer wasn't correct. Try again!
...
head(d)
#< hint
display("You can find all mutate commands in the info box (make sure your closing brackets are correct); then type: select(party, year, votes, everything())")
#>
```
...
#< quiz "five_percent"
question: Which parties did pass the 5% threshold in the 2003 election according to our calculation?
mc:
  - Edinstvo*
  - KPRF*
  - LDPR*
  ...
  - SPS
  - Yabloko
success: Great, all you answers are correct!
failure: Sorry, you did not select all correct answers. Try again!
...
#< hint
display("Copy the code from the previous code chunks and add the facet_wrap() command")
#>
```

#< quiz "1999election"
question: Have a look at the graphs above. How many parties participated in the 1999 election?
answer: 6
success: Exactly! Except for NDR all main parties participated.
failure: I'm sorry, your answer is not correct. Try again!
...
#< hint
disply("Fill the placeholders (in this order) with: the regression name, the dependent variable, the explanatory variable")
#>
```
...
#< quiz "OLS1"
question: Which sign should we expect the coefficient for NTV availability to have?
sc:
  - positive
  - negative*
success: Exactly!
failure: I'm sorry, your answer is not correct.
...
#< quiz "OLS1results"
question: How would a 20 percent DECREASE in NTV availability affect the voting results for Unity?
sc:
  - -1.80%
  - +0.18%
  - +7.20%*
  - -7.20%
success: Exactly, +1% NTV availability implies -0.36% voting results, therefore -20% implies +7.2%.
failure: I'm sorry, your answer is not correct. Note, that we are talking about a DECREASE in NTV availability here!
...
#< quiz "Delta1"
question: What correlation do Gorod and Watch_probit_1999 probably have?
sc:
  - positive correlation*
  - negative correlation
success: Exactly! Being in a city (Gorod = 1) probably increases NTV availability.
failure: I'm sorry, your answer is not correct.
...
#< hint
  display("Just write: 'Delta <- felm(Gorod ~ Watch_probit_1999, data = dat)'")
#>
```
...
#< quiz "signOVB"
question: When omitting Gorod - Do you think the coefficient for NTV availability is biased towards zero?
sc:
  - yes
  - no*
success: Exactly!
failure: I'm sorry, your guess is not correct. Try again or go straight for the summary below.
...
```{r "21_4", optional = TRUE}
1.3960*-2.2385

#< hint
display("Multiply the coefficient for Watch_probit_1999 in our Delta regression and the one for Gorod in our OLS_long.")
#>
```

#< quiz "OVB"
question: What omitted variable bias (up to 4 digits) do we expect?
answer: -3.1249
roundto: 0.0001
success: Perfect!
failure: I'm sorry, your answer is not correct. You can get a hint in the code chunk above!
...
#< hint
display("after nurses1998 add: '|region'")
#>
```
...
reg.sum(Unity, FELM1)
#< hint
display("simply insert the regression names for the placeholders")
#>
```
...
#< hint
display("use reg.sum as done before")
#>
```
...
#< hint
display("don't forget the summary")
#>
```
...
#< quiz "T2"
question: Mark all correct statements!
mc:
  - The presence of NTV encourages people to participate in the election.
  - An increase in NTV availability implies a decrease in the voting results for Unity and nationalist LDPR.*
  - The coefficient for NTV availability is significant for all parties.*
success: Great, you got all the right answers!
failure: I'm sorry, not all answers are correct. Try again!
...
#< hint
display("copy the code from above ad replace population1998 with wage98")
#>
```
...
#< quiz "Waldtest"
question: What is the joint significance level of the 1995 voting results in FELM4?
sc:
  - 1%
  - 5%
  - 10%
  - not significant*
success: Exactly! For a 10% significance level the p-value had to be below 0.1!
failure: I'm sorry, your answer was not correct. Have a closer look at the p-value (p.F)!
...
mc:
  ...
  - Socioeconomic characteristics are jointly still not significant*
  - Our determinants of transmitter location are jointly significant at the 1% level*
success: Great, all answers are correct!
failure: I'm sorry, you didn't get all correct answers. Try again!
...
#< hint
display("Use: 'dat[!is.na(dat$Votes_Edinstvo_1999),]' and assign it to 'dat',
        don't forget the comma for seperating row and column referencing in data frames!")
#>
```
...
head(d)
#< hint
display("insert 'party' and 'votes' into the spread() command")
#>
```
...
head(d)
#< hint
display("Use: d <- d %>% filter(year != ...) %>% select(tik_id, year, SPS, ....)")
#>
```
...
#< quiz "Placebo"
question: Does the outcome (consider LDPR not be not significant as in EPZ's analysis) imply fulfilment of our assumptions?
sc:
  - yes*
  - no
...
#< quiz "SPS_Yabloko"
question: Are the results consistent with our hypothesis?
sc:
  - yes*
  - no
success: Exactly!
failure: I'm sorry, that was the wrong decision.
